...
import { localeFromPath } from "@/config/paths";
import { getUi, useLocalePath, useUi } from "@/i18n";
import { buildMeta } from "@/lib/seo";

export const meta = ({ location }: { location: { pathname: string } }) => {
    const locale = localeFromPath(location.pathname);
    const ui = getUi(locale);
    return buildMeta({ title: ui.notFound.title, description: ui.notFound.description, target: { type: "notFound" }, locale, noindex: true });
};
...
export default function NotFound() {
    ...
    return (
        <div className="mx-auto max-w-5xl px-4 py-16 text-center md:px-8 md:py-24">
            ...
            <p className="mt-6 text-sm font-semibold text-brand-secondary">{ui.notFound.code}</p>
            ...
            <ButtonLink to={to({ type: "home" })} size="xl" className="mt-8">
                {ui.notFound.cta}
            </ButtonLink>
