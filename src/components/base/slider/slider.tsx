...
export const Slider = ({ label, labelPosition = "default", minValue = 0, maxValue = 100, labelFormatter, formatOptions, ...rest }: SliderProps) => {
    ...
    return (
        <AriaSlider {...rest} {...{ minValue, maxValue }} formatOptions={formatOptions ?? defaultFormatOptions}>
            <AriaLabel className="sr-only">{label}</AriaLabel>
            <AriaSliderTrack className="relative h-6 w-full">
                {({ state: { values, getThumbValue, getThumbPercent, getFormattedValue } }) => {
                    ...
                    return (
                        <>
                            <span className="absolute top-1/2 h-2 w-full -translate-y-1/2 rounded-full bg-brand-100" />
                            <span
                                ...
                                return (
                                    <AriaSliderThumb
                                        ...
                                        className={({ isFocusVisible, isDragging }) =>
                                            cx(
                                                "top-1/2 box-border size-6 cursor-grab rounded-full bg-slider-handle-bg shadow-[0_4px_12px_-2px_rgb(255_106_0/0.6)] ring-[3px] ring-slider-handle-border ring-inset transition-transform duration-150 hover:scale-110",
